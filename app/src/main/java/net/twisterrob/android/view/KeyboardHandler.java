...
public abstract class KeyboardHandler {
    ...
    public void registerEditText(EditText editText) {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN) {
            ...
            try {
                editText.getClass().getMethod("setShowSoftInputOnFocus", boolean.class).invoke(editText, false); //NON-NLS
            } catch (Exception ex) {
        ...
        public void onKey(int primaryCode, int[] keyCodes) {
            ...
            switch (primaryCode) {
                case KEY_CLEAR:
                    ...
                    break;
                case KEY_BACKSPACE: {
                    if (editable != null && 0 < start) editable.delete(start - 1, start);
                    break;
                }
                case KEY_DONE:
                    hideCustomKeyboard();
                ...
                case KEY_MOVE_END:
                    ...
                    break;
                case KEY_MOVE_LEFT: {
                    if (start > 0) editor.setSelection(start - 1);
                    break;
                }
                case KEY_MOVE_RIGHT: {
                    if (start < editor.length()) editor.setSelection(start + 1);
                    break;
                }
                default: {
